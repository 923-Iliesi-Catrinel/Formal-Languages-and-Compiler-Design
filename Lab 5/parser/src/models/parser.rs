...
impl Parser {
    ...
    fn parse_productions(
        ...
        loop {
            ...
            // TODO: perhaps don't count on the whitespaces being there
            // split the production string into a vector of productions
            let productions: Vec<String> = production.split(" | ").map(|s| s.to_string()).collect();

            // check if the productions vector is empty
